...
#include "PlayerList/BulletAttack.h"
...
void AttackManager::Bullet(Player* player)
{
	...
	// 弾の生成位置：プレイヤーの少し前方＆高さを補正
	Vector3 spawnPos = player->GetPosition() + dir * 1.5f + Vector3(0, 1.5f, 0);

	auto bullet = std::make_shared<BulletAttack>(
		spawnPos,
		dir,
		m_bulletModel,
		m_states,
...
void AttackManager::Rush(Player* player)
{
	...
	// ノックバックパワーの設定を追加
	// ダッシュ攻撃なので、近接攻撃(AttackP)より強めに設定する
	rushAttack->SetKnockbackPower(30.0f); // ノックバックパワー: 30.0f (例)
	...
	for (auto& atk : m_attacks)
	{
		...
		for (auto& enemy : enemies)
		{
			...
			if (atkCol->Intersects(enemyCol))
			{
				...
				// 弾（BulletAttack）だった場合
				if (dynamic_cast<BulletAttack*>(atk.get()))
				{
					//種類
